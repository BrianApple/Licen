...
| **防拷贝** | License 绑定**主板+CPU+MAC+磁盘+系统UUID**五要素 SHA-256 机器码（含虚拟机 DMI UUID，克隆 VM 也能区分），整套拷走也跑不起来 | 单纯 License 文件可随意复制 |
...
- **机器码绑定**：主板+CPU+主MAC+磁盘+系统UUID → SHA-256，License 无法迁移服务器（含虚拟机场景）

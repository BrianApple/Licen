...
// DefaultProduct 默认产品标识。
// 通用 SDK 为空字符串（Config.Product 未设置时走服务端宽松模式）；
// 从签发服务下载的定制版 SDK 会把占位符替换为所选产品 ID，
// 注册/心跳默认携带该产品自声明。
const DefaultProduct = "__LICEN_DEFAULT_PRODUCT__"
...
type Config struct {
	...
	// 应用密钥
	AppSecret string
	// 产品标识（自声明，如 hxapigate；须与管理端 App 绑定的产品一致，
	// 服务端三层校验：SDK 声明 == App 凭证 == License 签名授权）
	Product string
	// 节点名称（默认本机 hostname）
	NodeName string
	// 心跳间隔（默认 30s）
